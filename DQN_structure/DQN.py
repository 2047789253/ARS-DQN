...
class Net(nn.Module):
    ...
    #     # self.fc3 = nn.Linear(node_num_3*map_xdim*map_ydim, 512)
    #     self.fc3 = nn.Linear(node_num_4*int(map_xdim/4)*int(map_ydim/4), 256)
    #     # self.fc3 = nn.Linear(6144, 1024)  # 72
    #     # self.fc3 = nn.Linear(1024, 256)  # 54
    #     # self.fc3 = nn.Linear(3072, 1024)  # 36
    ...
    #     # print(len(x.view(x.size(0), -1)[0]))
    #     # print("x4-", x.view(x.size(0), -1))
...
class Agent:
    ...
    def choose_action(self, obs, current_place, target_place, valid_path_matrix, matrix_padding=0, use_astar=True):
        # self.epsilon = 1
        # self.epsilon = 0
        if np.random.uniform() < self.epsilon:  # greedy (利用：使用神经网络)
            ...
            action = torch.max(actions_value.cpu(), 1)[1].data.numpy()  # transform to CPU
            ...
            if use_astar:
                ...
                # 假设动作空间是 0:UP, 1:RIGHT, 2:DOWN, 3:LEFT, 4:STOP
                # 通常训练时不希望它一直 STOP，所以随机范围设为 0-3 或 0-4
